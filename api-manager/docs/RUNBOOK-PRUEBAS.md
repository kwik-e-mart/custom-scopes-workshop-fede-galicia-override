# Runbook de pruebas — service Api Manager

Recorrido **paso por paso** para probar a mano cada funcionalidad del service `Api Manager` contra
el CRC local. Cada paso dice qué correr, **qué tenés que ver** para darlo por bueno, y qué hacer si
no da eso.

**Qué asume:** el CRC local arriba con Kuadrant, Authorino (`clusterWide=true`) y Gateway API ya
instalados, el Gateway `s2s-ingress` y la `AuthPolicy` `s2s-validator` ya provistos (namespace
`gateways`), y los workloads `payments`/`other` de la demo S2S (en particular el echo server
`reports`, Service en el puerto 8080, path `/whoami`). Ese sustrato es el mismo que usa
`demo-kuadrant-s2s/RUNBOOK-PRUEBAS.md` — este documento no lo reprovisiona.

**Qué NO cubre:** el bootstrap del CRC ni la instalación de Kuadrant/Gateway API (eso es
`specs/prerequisites/` del service, y en este cluster ya está resuelto por el layer de la demo S2S —
ver Paso 2 sobre por qué no se re-aplica acá).

**Los resultados que figuran como "tenés que ver" son salidas reales**, medidas el 2026-08-31 y
actualizadas el 2026-09-01 (al agregarse la publicación GitOps) contra el POC
(`account_id=1374028000`, cluster CRC). Todo lo que en este documento se marca como **BLOQUEADO** es
exactamente eso: se intentó, se documenta la causa real, y no se inventó una salida.
